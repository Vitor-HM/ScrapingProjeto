...
p_tema_anime = st.sidebar.multiselect(
    "Filtre por tema: ",
    options=tema_unico
)

#Ativando o filtro
df_selecionado = df[df['genero'].str.contains('|'.join(p_genero), na=False) &
                  df['tema_anime'].str.contains('|'.join(p_tema_anime), na=False)]

df_estatistica_anime = df_selecionado[['id', 'nome', 'data_lancamento', 'tema_anime', 'genero']]
...
total_generos = len(generos_unicos) #procurar um jeito de fazer esse filtro ser dinamico.
...
with col2:
    st.metric(label="Total De Generos Lançados: ",value=total_generos) #valor que vai dentro da primeira coluna
...
total_animes_temporada = df_selecionado['estacoes'].value_counts().reset_index()
...
total_genero = df_selecionado.where(df_selecionado['genero'] != 'nao revelado').groupby('genero').size().reset_index()
total_genero.columns = ['generos', 'total_animes']
df_total_genero = total_genero.sort_values(by='total_animes',ascending=False).reset_index(drop=True).head(5)
...
df_animes_por_fonte = animes_por_fonte.sort_values(by='total_animes',ascending=False).reset_index(drop=True).head(5)
...
fig3 = px.scatter(df_selecionado, x='qtd_episodios', y='duracao', title='Relação entre Quantidade de Episódios e Duração Mínima',
                  labels={'qtd_episodios': 'Quantidade de Episódios', 'min_duracao': 'Duração Mínima (minutos)'})
